...
import static org.beta.authentification.keycloak.authenticator.OtpLoginAuthenticator.AUTH_NOTE_OTP;
...
public class OtpLoginAuthenticatorTest {
    ...
    class AuthenticateFlowTest {
        ...
        public void authenticate_should_not_send_otp_and_show_form_for_disabled_users() {
            ...
            verify(emailSender, times(0)).sendEmail(any(), any(), any(), anyString(), anyString());
            verify(botSender, times(0)).sendMessage(anyString(), anyString(), anyString(), anyInt());
        }
        ...
        public void authenticate_should_send_otp_and_show_form() {
            ...
            verify(emailSender, times(1))
                    .sendEmail(any(), any(), any(), eq(code), eq(String.valueOf(codeTimeout)));
            verify(botSender, times(1)).sendMessage(anyString(), anyString(), eq(code), anyInt());
        }
    }
    ...
    class AuthenticateFlowDisableBotTest {
        ...
        public void authenticate_should_not_send_otp_and_show_form_for_disabled_users() {
            ...
            verify(context, times(1)).challenge(any());
            verify(emailSender, times(0)).sendEmail(any(), any(), any(), anyString(), anyString());
            verify(botSender, times(0)).sendMessage(anyString(), anyString(), anyString(), 0);
        }
        ...
        public void authenticate_should_send_otp_and_show_form() {
            ...
            verify(emailSender, times(1))
                    .sendEmail(any(), any(), any(), eq(code), eq(String.valueOf(codeTimeout)));
            verify(botSender, times(0)).sendMessage(anyString(), anyString(), eq(code), 0);
        }
    }
    ...
    class ActionFlowTest {
        ...
        public void action_should_fail_with_invalid_credential_for_disabled_users() {
            ...
            verify(context, times(0)).success();
            verify(context, times(0)).challenge(any());
            verify(secureCode, times(0))
                    .isValid(anyString(), anyString(), anyString(), anyInt(), anyInt());
        }
