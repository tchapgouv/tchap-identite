...
public class SecureCode {
    ...
    /**
     * Splits the passed string using SEPARATOR to make it more readable split conditions: - don't
     * split if length <= 5 - split in 4s if divisible by 4 or length > 20 - split in 3s if
     * divisible by 3
     */
    public String makeCodeUserFriendly(String code) {
        ...
        for (int i = (codeLength - 1) / codeSnippetLength * codeSnippetLength;
                i > 0;
                i -= codeSnippetLength) {
            userFriendlyCode =
                    userFriendlyCode.substring(0, i) + SEPARATOR + userFriendlyCode.substring(i);
        }
    ...
    /**
     * Validates the code input by the user returns true if entered code met following conditions: -
     * code is equal to the code send out by email (ignoring SEPARATOR) - code got entered before
     * timeoutInMinutes have passed since code got sent - code got entered after
     * codeActivationDelayInSeconds have passed since code got sent
     */
    public boolean isValid(
        ...
        boolean codeActive =
                timePassedSinceRequest < 1000 * 60 * timeoutInMinutes
